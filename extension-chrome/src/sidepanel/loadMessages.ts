/**
 * Synchronously import every locale bundle so the side panel can switch
 * languages on the fly without a network round-trip. Vite includes all
 * imports in the chunk, but it's only ~30 KB minified+gzipped which is
 * well under the 200 KB budget for the side panel.
 */
...
/** English keys merged on top of the active bundle (same strategy as web). */
